...
<template>
    <div class="reply card" style="margin-top: 10px;margin-bottom: 10px;">
        <div>
            <UserSign font-color="black" show-tag :uid="$props.item.sender"></UserSign>
            <span>{{ new Date(item.sendTime).toLocaleString() }}</span>
            <span style="color: rgba(10,10,10,.3);margin-left: 10px;" :id="item.id">id:{{ item.id }}</span>
        ...
        <div v-if="showedReply" style="width: 100%;padding: 5px;margin: -5px -10px;background-color: rgb(239, 239, 239);">
